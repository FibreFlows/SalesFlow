...
export function PilotClient() {
  ...
  const [signupProfile, setSignupProfile] = useState({ full_name: "Sahil Bhatia", tech_id: "X2002709", afl_email: "sahil.bhatia@aflglabal.com", telus_email: "sahil.bhatia1@telus.com" });
  ...
  async function submitLead(event: FormEvent<HTMLFormElement>) {
    ...
    if (editingId) {
      ...
      const installationCompleted = form.installation_completed === "" ? null : form.installation_completed === "yes";
      const lead = { id: crypto.randomUUID(), owner_id: user.id, ...leadForm, estimated_value: 0, possible_rgu_sale: Number(form.possible_rgu_sale || 0) || null, current_monthly_cost: Number(form.current_monthly_cost || 0), customer_rating: Number(form.customer_rating || 0) || null, installation_completed: installationCompleted, converted_at: convertedAt, installation_completed_at: installationCompleted ? now : null, contract_expiry_date: form.contract_expiry_date || null, last_contacted_at: form.last_contacted_at || null, next_follow_up_at: form.next_follow_up_at || null, created_at: now, updated_at: now };
  ...
  if (ready && !user) return <main className="grid min-h-screen place-items-center bg-background p-5 text-foreground"><Card className="w-full max-w-md"><CardHeader><Badge className="w-fit border-primary/30 bg-primary/10 text-primary">CLOUD DATABASE</Badge><CardTitle className="pt-3 text-2xl">Sign in to SalesFlow</CardTitle></CardHeader><CardContent className="space-y-3"><Input type="email" placeholder="Login email" value={email} onChange={(e) => setEmail(e.target.value)} /><Input type="password" placeholder="Password" value={password} onChange={(e) => setPassword(e.target.value)} /><div className="rounded-md border border-border p-3"><p className="mb-2 text-xs font-medium text-muted-foreground">Account details used on reports</p><div className="space-y-2"><Input placeholder="Full name" value={signupProfile.full_name} onChange={(e) => setSignupProfile({ ...signupProfile, full_name: e.target.value })} /><Input placeholder="Tech ID" value={signupProfile.tech_id} onChange={(e) => setSignupProfile({ ...signupProfile, tech_id: e.target.value })} /><Input type="email" placeholder="AFL email" value={signupProfile.afl_email} onChange={(e) => setSignupProfile({ ...signupProfile, afl_email: e.target.value })} /><Input type="email" placeholder="TELUS email" value={signupProfile.telus_email} onChange={(e) => setSignupProfile({ ...signupProfile, telus_email: e.target.value })} /></div></div>{message ? <p className="text-sm text-primary">{message}</p> : null}<Button className="w-full" onClick={() => authenticate("signin")}>Sign in</Button><Button className="w-full" variant="outline" onClick={() => authenticate("signup")}>Create account with these details</Button></CardContent></Card></main>;
