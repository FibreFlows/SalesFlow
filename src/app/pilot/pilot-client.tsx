...
export function PilotClient({ entryOnly = false, listOnly = false }: { entryOnly?: boolean; listOnly?: boolean } = {}) {
    ...
    <main className="min-h-screen bg-background p-5 text-foreground sm:p-8">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-6 flex flex-col justify-between gap-4 md:flex-row md:items-end">
          <div><Badge className="border-primary/30 bg-primary/10 text-primary">SALES LEADS</Badge><h1 className="mt-3 text-3xl font-semibold tracking-tight">{entryOnly ? (editingId ? "Edit sales lead" : "New sales lead") : "Sales lead tracker"}</h1><p className="mt-2 text-sm text-muted-foreground">{entryOnly ? "Add the customer details you have. Every field is optional." : "Search, review and manage all sales lead records."}</p></div>
        ...
        <div className={`${entryOnly ? "hidden" : "grid"} mb-6 gap-3 sm:grid-cols-2 xl:grid-cols-4`}>
        ...
        {!entryOnly && !listOnly ? <div className="mb-5 grid grid-cols-2 rounded-2xl border border-border bg-muted/60 p-1 xl:hidden"><button onClick={() => setMobileView("add")} className={`flex min-h-11 items-center justify-center gap-2 rounded-xl text-sm font-semibold transition ${mobileView === "add" ? "bg-card text-primary shadow-sm" : "text-muted-foreground"}`}><UserPlus className="size-4" /> Add lead</button><button onClick={() => setMobileView("list")} className={`flex min-h-11 items-center justify-center gap-2 rounded-xl text-sm font-semibold transition ${mobileView === "list" ? "bg-card text-primary shadow-sm" : "text-muted-foreground"}`}><List className="size-4" /> Lead list</button></div> : null}

        <div className={`grid gap-6 ${entryOnly || listOnly ? "grid-cols-1" : "xl:grid-cols-[380px_1fr]"}`}>
          <Card className={`${listOnly ? "hidden" : entryOnly ? "block" : mobileView === "add" ? "block" : "hidden"} mx-auto w-full ${entryOnly ? "max-w-3xl" : ""} rounded-[22px] ${entryOnly || listOnly ? "" : "xl:block"}`}>
          ...
          <Card className={`${entryOnly ? "hidden" : listOnly ? "block" : mobileView === "list" ? "block" : "hidden"} rounded-[22px] ${entryOnly || listOnly ? "" : "xl:block"}`}>
            <CardHeader className="gap-4 sm:flex-row sm:items-center sm:justify-between"><div><CardTitle>Leads</CardTitle><p className="mt-1 text-sm text-muted-foreground">{leads.length} saved record{leads.length === 1 ? "" : "s"}</p></div><div className="flex flex-col gap-2 sm:flex-row"><Link href="/entry?type=lead"><Button className="w-full sm:w-auto"><Plus /> Add lead</Button></Link><div className="relative sm:w-72"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground" /><Input className="pl-9" placeholder="Search leads..." value={query} onChange={(e) => setQuery(e.target.value)} /></div></div></CardHeader>
            <CardContent>
