...
type Job = { id: string; customer_name: string | null; scheduled_date: string; specification: string | null; status: string; sale_done_by_me: boolean | null; sale_converted_at: string | null; sales_count: number; telus_sims_sold: number; koodo_sims_sold: number; lead_data: { sale_scope?: string[] } | null };
type Lead = { id: string; first_name: string; last_name: string; company: string; status: string; possible_rgu_sale: number | null; sale_scope: string[]; created_at: string; converted_at: string | null; sales_count: number; telus_sims_sold: number; koodo_sims_sold: number };
...
export function ReportsClient() {
  ...
  const report = useMemo(() => { const start = periodStart(period); const periodJobs = jobs.filter((job) => jobInPeriod(job.scheduled_date, start)); const leadSales = leads.filter((lead) => inPeriod(lead.converted_at, start)); const jobSales = jobs.filter((job) => inPeriod(job.sale_converted_at, start)); const possible = leads.filter((lead) => lead.status !== "converted" && lead.status !== "unqualified" && inPeriod(lead.created_at, start)); const salesTotal = leadSales.reduce((sum, lead) => sum + Math.max(Number(lead.sales_count || 0), 1), 0) + jobSales.reduce((sum, job) => sum + Math.max(Number(job.sales_count || 0), 1), 0); return { start, periodJobs, leadSales, jobSales, possible, salesTotal }; }, [period, jobs, leads]);
  const exportPdf = () => { const lines = ["SALESFLOW ACTIVITY REPORT", `${period.toUpperCase()} REPORT`, "", `Name: ${profile.full_name}`, `Tech ID: ${profile.tech_id}`, `AFL Email: ${profile.afl_email}`, `TELUS Email: ${profile.telus_email}`, `Period starting: ${report.start.toLocaleDateString()}`, `Generated: ${new Date().toLocaleString()}`, "", `Workload jobs: ${report.periodJobs.length}`, `Sales units: ${report.salesTotal}`, `Possible leads: ${report.possible.length}`, "", "WORKLOAD", ...report.periodJobs.map((job) => `${job.scheduled_date} | ${job.customer_name || "Unnamed customer"} | ${job.specification || "No specification"} | ${job.status}`), "", "CONVERTED SALES", ...report.leadSales.map((lead) => `${lead.converted_at?.slice(0, 10)} | ${lead.first_name} ${lead.last_name} | ${lead.sales_count || 0} sales | TELUS SIMs ${lead.telus_sims_sold || 0} | Koodo SIMs ${lead.koodo_sims_sold || 0} | Sold: ${lead.sale_scope?.join(", ") || "Not specified"} | Add Lead`), ...report.jobSales.map((job) => `${job.sale_converted_at?.slice(0, 10)} | ${job.customer_name || "Unnamed customer"} | ${job.sales_count || 0} sales | TELUS SIMs ${job.telus_sims_sold || 0} | Koodo SIMs ${job.koodo_sims_sold || 0} | Sold: ${job.lead_data?.sale_scope?.join(", ") || "Not specified"} | Workload`), "", "NOT SOLD BUT POSSIBLE", ...report.possible.map((lead) => `${lead.first_name} ${lead.last_name} | Possible: ${lead.sale_scope?.join(", ") || "Not specified"} | RGU ${lead.possible_rgu_sale || 0}`)]; downloadPdf(lines, `salesflow-${period}-report-${new Date().toISOString().slice(0, 10)}.pdf`); };
  return <main className="min-h-screen bg-background p-5 text-foreground sm:p-8"><div className="mx-auto max-w-6xl"><Link href="/" className="mb-6 inline-flex items-center gap-2 text-sm text-muted-foreground"><ArrowLeft className="size-4"/> Dashboard</Link><div className="flex flex-col justify-between gap-4 sm:flex-row sm:items-end"><div><Badge className="border-primary/30 bg-primary/10 text-primary">LIVE CLOUD REPORTS</Badge><h1 className="mt-3 text-3xl font-semibold">Sales and workload reports</h1><p className="mt-2 text-sm text-muted-foreground">{profile.full_name} · {profile.tech_id} · {profile.afl_email} · {profile.telus_email}</p></div><Button onClick={exportPdf}><Download/> Download PDF</Button></div><div className="mt-6 flex flex-wrap gap-2">{(["daily","weekly","monthly","yearly"] as Period[]).map((item)=><Button key={item} variant={period===item?"default":"outline"} onClick={()=>setPeriod(item)} className="capitalize">{item}</Button>)}</div><section className="mt-6 grid gap-4 sm:grid-cols-3"><Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">Workload jobs</p><p className="mt-2 text-3xl font-semibold">{report.periodJobs.length}</p></CardContent></Card><Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">Converted sales</p><p className="mt-2 text-3xl font-semibold">{report.salesTotal}</p></CardContent></Card><Card><CardContent className="p-5"><p className="text-sm text-muted-foreground">Not sold but possible</p><p className="mt-2 text-3xl font-semibold">{report.possible.length}</p></CardContent></Card></section><Card className="mt-6"><CardHeader><CardTitle className="flex items-center gap-2"><FileText className="size-5"/> {period[0].toUpperCase()+period.slice(1)} details</CardTitle></CardHeader><CardContent className="space-y-5"><div><h2 className="font-medium">Workload jobs</h2>{report.periodJobs.length?<div className="mt-2 space-y-2">{report.periodJobs.map((job)=><div key={job.id} className="rounded-md border p-3 text-sm"><p className="font-medium">{job.customer_name||"Unnamed customer"}</p><p className="text-muted-foreground">{job.scheduled_date} · {job.specification||"No specification"} · {job.status}</p></div>)}</div>:<p className="mt-2 text-sm text-muted-foreground">No jobs in this period.</p>}</div><div><h2 className="font-medium">Converted sales</h2><p className="mt-2 text-sm text-muted-foreground">{report.salesTotal} sales units converted in this period.</p></div><div><h2 className="font-medium">Not sold but possible</h2><p className="mt-2 text-sm text-muted-foreground">{report.possible.length} possible leads added in this period.</p></div></CardContent></Card></div></main>;
}
