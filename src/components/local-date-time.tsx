...
export function LocalDateTime() {
  ...
  const [profile, setProfile] = useState({ full_name: "Sahil Bhatia", bio: "Fibre sales and customer experience professional." });
  ...
  useEffect(() => {
    const load = async () => {
      ...
      if (data) setProfile({ full_name: data.full_name || "Sahil Bhatia", bio: data.bio || "Fibre sales and customer experience professional." });
    };
    const frame = requestAnimationFrame(load);
    return () => cancelAnimationFrame(frame);
  }, []);

  const firstName = profile.full_name.trim().split(/\s+/)[0] || "Sahil";
  return <div><p className="mb-2 text-xs font-bold uppercase tracking-[.08em] text-primary sm:text-sm">{parts ? `${parts.date} · ${parts.time}` : "Edmonton local time"}</p><h1 className="text-[2rem] font-semibold tracking-tight sm:text-4xl">{parts?.greeting || "Welcome"}, {firstName}.</h1><div className="mt-1.5 flex items-start gap-2 sm:mt-2"><p className="max-w-xl text-sm text-muted-foreground sm:text-base">{profile.bio}</p><Link href="/profile" aria-label="Edit dashboard bio" className="mt-0.5 rounded-full p-1 text-primary hover:bg-primary/10"><Pencil className="size-3.5" /></Link></div></div>;
}
